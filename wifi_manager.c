...
wifi_mode_t wifi_connect_or_start_ap(const char *ssid, const char *password) {
    ...
    if (cyw43_arch_wifi_connect_timeout_ms(ssid, password,
        ...
        LOGF("Wi-Fi station connected\n");
        return WIFI_MODE_STA;
    }

    ERRF("Wi-Fi connect failed. Starting AP with SSID: %s\n", ssid);
    cyw43_arch_enable_ap_mode(ssid, password, CYW43_AUTH_WPA2_AES_PSK);
    ERRF("AP mode started. DHCP server not available in this SDK install.\n");
    ERRF("Set phone IP manually: 192.168.4.2/24, gateway: 192.168.4.1\n");
    return WIFI_MODE_AP;
}
