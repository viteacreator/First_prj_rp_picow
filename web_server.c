...
static void build_app_js(char *out, size_t out_len) {
    snprintf(out, out_len,
        ...
        "var max=Math.max.apply(null,sp.concat(y).concat(u).concat(u1).concat([1]));"
        "var min=Math.min.apply(null,sp.concat(y).concat(u).concat(u1).concat([0]));"
        "var range=(max-min)||1;"
        ...
        "if(q('show_u1').checked)plot(u1,'#6abf4b');"
